...
{
	//TODO: IMPLEMENT ME!
	Process* thiss = (Process*) this;
	Process* thatt = (Process*) that;
	if (thiss->priority > thatt->priority){
		return -1;
	}
	else if (thiss->priority == thatt->priority){
		if (thiss-> arrival_time > thatt->arrival_time){
			return 1;
		}
		return -1;
	}
